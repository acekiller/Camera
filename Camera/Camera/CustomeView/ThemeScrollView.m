...
- (void)initializa{
    ...
    // load theme data for TuTuTheme.plist file
    NSString *plistPath  = [[NSBundle mainBundle] pathForResource:@"Frames" ofType:@"plist"];
    NSMutableArray *temp = [NSMutableArray array];
    NSMutableArray *themImagesArray = [NSMutableArray arrayWithContentsOfFile:plistPath];
...
- (void)setThemeImages:(NSMutableArray *)themeImages{
    // 判断新旧主题是否一直
    if (themeImages == _themeImages) {
        return;
    }
    
    // 移除所有的主题视图
    if (nil != _themeImages) {
        int count = [_themeImages count];
        [_themeImages  removeAllObjects];
...
- (void)loadThemeImages{
    ...
    NSInteger index = 0;
    for (ThemeMaterial *matl in _themeImages) {
        NSString *imagePath  = [[NSBundle mainBundle] pathForResource:matl.thumbImageName ofType:nil];
        NSString *text       = matl.themeName;
        UIImage  *themeImage = [UIImage imageWithContentsOfFile:imagePath];
        
        CGPoint centerPt;
        if (nil != themeImage) {
            // 加载主题视图
            ThemeImageView *themeView = [[ThemeImageView alloc] initWithFrame:CGRectMake(0, 0, 72, 72)];
            themeView.themeName = text;
            themeView.thumbImageName = matl.thumbImageName;
     ...
     xPosition = xPosition > 320.0f ? xPosition : 321.0f;
...
- (void)setCurrentSelectedItem: (int) index {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(themeScrollView:didSelectMaterila:)]) {
        [self.delegate themeScrollView:self
                     didSelectMaterila:[self.themeImages objectAtIndex:tmp.tag - THEME_TAG_BASE]];
    }
}

- (void)scrollToItemAtIndex:(int)index {
	if (index<0 || index >= [_themeImages count]) return;	// 不符合索引直接返回
